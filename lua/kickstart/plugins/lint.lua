...
local function find_linter_config(cfg_files, opts)
  opts = opts or {}
  ...
  local found = vim.fs.find(cfg_files, {
    upward = true,
    path = current_dir,
    stop = uv.os_homedir(), -- Optional: stop at home dir to avoid scanning root
  })

  -- 3. If found, return the first match (it will be the closest one)
  if #found > 0 then return found[1] end

  if not opts.global_fallback then return end

  -- 4. Fallback to your global config
  -- stdpath('config') points to your ~/.config/nvim/
  for _, cfg_file in ipairs(cfg_files) do
    local global_cfg = vim.fs.joinpath(vim.fn.stdpath 'config', cfg_file)
    if uv.fs_stat(global_cfg) then return global_cfg end
  end
end
...
return {
  { -- Linting
    ...
    config = function()
      ...
      local luacheck_config = find_linter_config({ '.luacheckrc' }, { global_fallback = true })
      if luacheck_config then
        luacheck.args = {
          '--config',
          luacheck_config,
      ...
      local golangci_config = find_linter_config({
        '.golangci.yml',
        '.golangci.yaml',
        '.golangci.toml',
        '.golangci.json',
      ...
      if golangci_config then
        table.insert(lint.linters.golangcilint.args, 1, golangci_config)
